...
resource "aws_key_pair" "folio_key_pair" {
...
resource "aws_security_group" "rancher_sg_allowall" {
  ...
  description = "Rancher folio - allow all traffic"
  ...
  tags = {
    Creator = "rancher-folio"
  }
}

resource "aws_iam_instance_profile" "rancher-profile" {
  name  = "rancher-profile"
  role  = aws_iam_role.rancher-role.name
}
...
resource "aws_instance" "rancher_server" {
  ...
  iam_instance_profile  = aws_iam_instance_profile.rancher-profile.name

  key_name        = aws_key_pair.folio_key_pair.key_name
...
module "rancher_common" {
  ...
  workload_cluster_name       = "folio-aws-custom"
}

# AWS EC2 instance for creating a single node workload cluster
resource "aws_instance" "folio_node" {
  ...
  user_data = templatefile(
    join("/", [path.module, "files/userdata_folio_node.template"]),
    {
      docker_version   = var.docker_version
      username         = local.node_username
